...
| `v031-facts-local-qwen9b-500` | 2026-09-22 | 0.3.1 release (0.3.2-SNAPSHOT for the 22 questions that hit the expectedKinds NPE, fixed in 72fa16c), proposer `lmstudio:qwen/qwen3.5-9b` Q4_K_M at 32k context | structured probe + fact and event keys + BM25 over observations, RRF (semantic channel off) | session | **0.889** [0.864, 0.911] | **0.919** [0.896, 0.939] | 33 ms / 93 ms | 199 s (4 local slots) | 3,835 |
...
### The full 500 on the 0.3.1 release (`v031-facts-local-qwen9b-500`, 2026-09-22): no regression

The same 500 questions, the same 9B proposer, on the release code: thirteen days of engine work since
`m3-facts-local-qwen9b-500` (seven tools, vocabulary registered from use, family Q, namesakes, possessives,
nested places). Paired against `m3-facts-local-qwen9b-500`, n=470: recall@5 **+0.004 [−0.017, +0.026]**,
recall@10 **+0.004 [−0.013, +0.021]**, so nothing gained and nothing lost at the retrieval level. Paired
against `m3-lexical`: recall@5 **+0.030 [+0.009, +0.053]**, recall@10 **+0.034 [+0.015, +0.055]**, the same
gain as in September's first run. Temporal-reasoning 0.778 at @5 (0.758 before), knowledge-update 0.965,
single-session-user 0.984. Recall p50 33 ms against 3 ms in the earlier run, with 288 facts a question on
average; the probe does more per question now (possessors, predicate groups, namesakes) and the cost has
not been profiled. The spec had changed since the first run, so 22,884 proposals were regenerated by the 9B, 164 of
them failing on context size at 32k; 22 questions errored on a null end date in `expectedKinds` and were
filled on a rerun with the fix, from the cache.
...
Opus 5.5, released the day before, on the same fifteen scenarios (`usage-opus-55`, 2026-09-23, 0.3.2
runner jar): 27 of 27, all four abstentions right, no protocol slips, no tool errors, the guide fetched in
all fifteen scenarios, 1.7 tool calls a step, about $1.83 (301k input, 1.09M cache reads, 18k output at
$4 / $0.20 / $20). One statement of 26 was not written down, and on purpose: "the benchmark data lives under
bench/data" was declined as repository layout that belongs in the project, which is what the instructions
say; Sonnet and Fable stored it, and Fable had declined it once before (`usage-fable`). The step said what the
instructions forbid, so it was changed the same day to a decision ("we decided to keep the benchmark data out
of the repository and fetch it on first run"); runs before 2026-09-23 saw the old wording.
Recall in the answering turn for 25 of 27 questions: "how should someone reach me if it is urgent" was
answered from the briefing it had already fetched in that conversation, and "did I ever say I lived
somewhere else" was answered through `inspect` with history on the owner rather than `recall`, correctly,
from the corrected fact's ledger. After a break, 5 of 7 recalled in the answering turn and all 7 answered
from the store.

The script grew again on 2026-09-23 to twenty-three scenarios, 114 steps and 63 questions (11 abstentions,
eight session breaks): a pasted mortgage email whose rate improves in a later email, a code base (who owns
what, a release decision, what a flaky test turned out to be; conventions are left to the repository), a car
from leaning to order, a home purchase with a handover that moves, a wedding timeline whose date moves, a
doctor and an allergy, a trip, and a child's school start with a corrected teacher. Runs from here on are
out of 63, not 27.

Opus 5.5 on the grown script (`usage-opus-55-2`, 2026-09-23) stopped after 72 of 114 steps when the API
account reached its monthly usage limit; to that point 36 of 36 answers were right, all statements written
down, no slips, no tool errors, and the two new scenarios it reached (the mortgage email, the code base)
passed whole. Four store behaviours came out of those traces and are fixed the same day: the mortgage email
raised three `event_effect` questions ("does mortgage_renewal start owns?") and the confirmation email a
fourth, which cost the first step its reply at the call limit, so a new event type is now asked about only
when a one-valued relation between its participants could be opened or closed, or a single person ended
(the reply still says how to give the type effects with `correct`); "what is the outstanding balance on the
mortgage" got an events verdict claiming the renewal "matches the question and is the answer" because the
type's word matched the loan's name, so the words that name a spotted thing or its kind no longer cue an
event; "who owns the Kestrel parser module" was a bare MISS while `responsible_for` held the answer, so a
miss now names the relations the thing is on record under; and a qualifier on `prefers` ("for anything
urgent, over email") was stored but never rendered and warned about at every run, so the preference-like
seeds gained the qualifier slot (V034). The run resumed from its finished scenarios with `--resume true`
on the rebuilt jar and finished: **63 of 63**, all 11 abstentions right, all 43 statements written down, a
recall in the answering turn for 60 of 63 (the other three answered from a recall earlier in the same
conversation), 17 of 20 after a break and all 20 from the store, no slips, no tool errors, 2.0 tool calls a
step. The resumed 42 steps cost about $3.50 (670k input, 951k cache reads, 31k output); the first 72 were
billed before the limit and not summed. Two more things the last six scenarios showed: the gate still asked
about every event between two people (a rehearsal dinner "starts spouse_of?") because kinship relations are
one-valued, so the kinship group is now left out of it, its changes being seeded events; and "what colour
did I order" got an events verdict that "matches the question and is the answer" because "ordered" cued the
order, so the verdict now says the events answer as far as they state what is asked. Opus itself set
